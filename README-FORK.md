...
| **Process supervision** | Deploy writes `deepseek-proxy.service`: **`Restart=always`**, `network-online`, burst limits; **`ExecStart`** passes **`--display-reasoning --no-collapsible-reasoning`** (collapsible is a no-op; thinking is mirrored as **`> 💭`** Markdown blockquotes), **`--base-url`**, **`--request-timeout`** (default **86400** s), **`--max-request-body-bytes`** (default **128 MiB**) so EC2 matches `config.py` even with a stale `~/.deepseek-cursor-proxy/config.yaml`. Override with **`PROXY_UPSTREAM_BASE_URL`**, **`PROXY_REQUEST_TIMEOUT`**, **`PROXY_MAX_BODY_BYTES`** when running deploy. |
...
- **Fork defaults win locally:** if you omit CLI flags, `main()` applies constants from `config.py` for **display/collapsible reasoning**, **upstream timeout**, **max body size**, **thinking**, **reasoning effort**, and **stream keepalive** — so stale YAML does not override them. Pass explicit flags (as the EC2 `ExecStart` does) to change any one value.
